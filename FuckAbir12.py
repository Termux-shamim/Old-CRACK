# Decompile by Nahid Afridy(Tools By Cobra-Linux Nahid afridy)
# Dont ader restiment with Nahid
...
import time
os.system("clear")
print("  ==> \033[1;32mTHIS TOOLS DECODE BY (SHAMIM HOSSEN ) ")
time.sleep(5)
...
import base64
import subprocess
from concurrent.futures import ThreadPoolExecutAB
def runtxt(z):
    for e in z + "\n":
        sys.stdout.write(e)
        sys.stdout.flush()
...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://github.com/Termux-shamim/Old-CRACK/blob/main/Appraval.%20txt"])
	exit(" [*] FACEBOOK : https://www.facebook.com/white.hat.hacker.Shamim ")
...
def notice():
	...
	runtxt("\033[0;93m SEND THIS KEY TO ADMIN FOR \033[0;96m  APPROVAL>> %s%s"%(G,basesplit))
	runtxt("\033[0;92m BYPASS ADMIN FACEBOOK  >> https://www.facebook.com/white.hat.hacker.Shamim")
	runtxt("\033[0;95m    <===================☠️  \033[0;97mEXIT\033[0;95m ☠️===================> ")
	subprocess.check_output(["am", "start", "https://www.facebook.com/white.hat.hacker.Shamim"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/Termux-shamim/Old-CRACK/blob/main/Appraval.%20txt').text
			if basesplit in plr:
				key = basesplit
		...
		print("\n"+C+"░█████╗░██████╗░██╗██████╗░\n██╔══██╗██╔══██╗██║██╔══██╗ "+Y+"|Esan"+C+"\n███████║██████╦╝██║██████╔╝ "+Y+"|Ahmed"+C+"\n██╔══██║██╔══██╗██║██╔══██╗ "+Y+"|Abir"+C+"\n██║░░██║██████╦╝██║██║░░██║\n╚═╝░░╚═╝╚═════╝░╚═╝╚═╝░░╚═╝\n "+N+"──────────────────────────────────────────────────"+G+"\n▸ AUTHOR     : Termux-Shamim\n▸ FACEBOOK   : https://www.facebook.com/white.hat.hacker.Shamim\n▸ YOUTUBE    : YOUTUBE.COM/ShamimTheTechVhai\n▸ GITHUB     : GITHUB.COM/Termux-Shamim\n"+N+"─────────────────────────────────────────────────\n")
	...
	def v2(self):
		if	os.path.isfile('.v2.py'):
			...
			os.system("pip install requests && pip install bs4 && clear && wget https://www.facebook.com/white.hat.hacker.Shamim")

		else:
			os.system("pkg install wget -y")
			os.system("wget https://www.facebook.com/white.hat.hacker.Shamim")
	...
	def v3(self):
		if	os.path.isfile('.v3.py'):
			...
			os.system("pkg install wget -y")
			os.system("wgethttps://www.facebook.com/white.hat.hacker.Shamim")
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [BY] : %s/%s -> \033[0;92m [ SHAMIM -OK:%s ]- \033[0;91m[BY-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
